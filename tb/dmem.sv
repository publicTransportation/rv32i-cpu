...
wire [XLEN-1:0] word_idx = dmem_addr >> 2;  // Convert byte address to word index (divide by 4); Wire is continuous assignment

always_ff @(posedge clk) begin
    // Synchronous Write
    if (word_idx < MEM_DEPTH) begin // Boundary check
        if (dmem_wmask[0]) mem[word_idx][ 7: 0] <= dmem_wdata[ 7: 0];
        if (dmem_wmask[1]) mem[word_idx][15: 8] <= dmem_wdata[15: 8];
        if (dmem_wmask[2]) mem[word_idx][23:16] <= dmem_wdata[23:16];
        if (dmem_wmask[3]) mem[word_idx][31:24] <= dmem_wdata[31:24];
    end
end
