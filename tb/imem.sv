...
#(
    ...
    //parameter HEX_PATH = "../tb/test.hex" // Compiled machine code filepath  
)(
...
initial begin // At the start of EACH simulation
    if (!$value$plusargs("HEX=%s", hex_file)) begin // Check CLine arg exists
        $display("FATAL: +HEX plusarg missing!");
        $finish;
    end

    $readmemh(hex_file, mem); // Load string (hex) into imem
