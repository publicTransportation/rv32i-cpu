...
always_comb begin // ALUOp mapping (ALUOp = 1X, X1, 00)
    ...
    if (alu_op[1]) begin
        case (funct3)
            3'b000:  alu_ctrl = (bit30 && !is_rtype) ? ALU_SUB : ALU_ADD; // SUB only for R-type (I-type instructions do NOT have a funct7)
            3'b001:  alu_ctrl = ALU_SLL;
            3'b010:  alu_ctrl = ALU_SLT;   // Handles SLT and SLTI
            3'b011:  alu_ctrl = ALU_SLTU;  // Handles SLTU and SLTIU
            3'b100:  alu_ctrl = ALU_XOR;
            3'b101:  alu_ctrl = bit30 ? ALU_SRA : ALU_SRL; // SRAI / SRA use bit 30 of instr
